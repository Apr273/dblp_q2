...
public class Introducer {
    ...
    public static void addMember(Integer ipToAdd) {
        ...
        memberList.memberAdd(String.valueOf(timestamp), ipToAdd);
    ...
    static class RunnableBroadCast implements Runnable {
        ...
        public void sendMessage() throws UnknownHostException {
            InetAddress address = InetAddress.getByName(baseInfo.introducerIp);//新节点ip地址
            byte[] data = (timeStamp + " " + ip).getBytes();//发送新节点ip
            //创建数据报，包含发送的数据信息
            DatagramPacket packet = new DatagramPacket(data, data.length, address, baseInfo.broadcastPort);
    ...
    static class ReceiveChange implements Runnable {
        ...
        public void run() {
            while (true) {
                //创建数据报，用于接收客户端发送的数据
                try (DatagramSocket socket = new DatagramSocket(baseInfo.introducerListPort)) {
                    ...
                    if (words[2].equals("failure") || words[2].equals("leave")) {
                        memberList.memberRemove(Integer.parseInt(words[3]));
                    }
                } catch (IOException e) {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
             ...
             */
                ...
                DatagramSocket socket = new DatagramSocket(baseInfo.introducerPort)) {
